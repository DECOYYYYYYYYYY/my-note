...
> 贪婪匹配：`* + ? {}` 限定符默认为贪婪匹配（匹配尽量多的字符），在它们后面加 `?` ，将改为非贪婪匹配（匹配尽量少的字符，注意是从此处向后匹配尽可能少的字符，不会向前删除已匹配的字符使得匹配的字符最少）
...
- `open(url, 标识符)`：打开新页面
  - 标识符：一个不含空格的字符串，用于指定加载资源的浏览上下文的名称。还可以使用特殊的 `target` 关键字：`_self`、`_blank`、`_parent` 和 `_top`。
